...
def retrieve_images_URLs(s3_bucket, import_id) -> list:
...
def generate_dataframe(processed_data_batch):
    ...
    for image_data in processed_data_batch:
        ...
        for box in boxes:
            data.append({"photo": name, "boxes": ", ".join(map(str, box.tolist()))})
...
def process(model: YOLO, s3_bucket, import_id, task_id):
    ...
    results_data_frame = generate_dataframe(results)
